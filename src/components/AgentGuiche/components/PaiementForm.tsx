...
export const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  // Ajoute un état pour stocker le montant de base
  const [baseApayer, setBaseApayer] = useState(0);
  ...
  const FetApayer = async () => {
    ...
    try {
      ...
      setBaseApayer(data.MontantApayer);
    } catch (error) {
      console.error(
        "Erreur lors de la récupération du montant à payer :",
        error
      );
    }
  };
  ...
  const handlePayer = async (value: MontantSaisi) => {
    ...
    try {
      const enregistrement = await ChangementRdvPaiement(
        dataClient?.ClientId,
        session?.user?.id,
        donnees
      );
                        ...
                        <FormControl>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              field.onChange(checked); // update RHF
                              SetApayer(
                                checked ? baseApayer + 1000 : baseApayer
                              );
                            }}
                            onBlur={field.onBlur}
                            name={field.name}
                        ...
                        <FormControl>
                          <Select
                            onValueChange={(value) => {
                              ...
                              if (value !== "wallet")
                                form.setValue("Wallet", undefined); // Reset Wallet if needed
                            }}
                            value={field.value}
                          >
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <Input
                                {...field}
                                value={field.value}
                                placeholder="Entre le Nom de la banque ..."
                              />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
      ...
      <Dialog open={isSucessOpen} onOpenChange={setisSucessOpen}>
        <DialogContent className="p-8 bg-white rounded-xl shadow-lg max-w-[980px] mx-auto">
          ...
          <ScrollArea className="h-[600px] w-full rounded-md border">
            {/* 🖨️ Zone à imprimer deux fois */}
            <div id="print-area" ref={printAreaRef} className="print:w-full">
              <Receipt
                dataClient={dataClient}
                donnees={donnees}
          ...
          <div className="flex justify-end mt-6">
            <Button onClick={handleImprimary}>Imprimer</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSummaryOpen} onOpenChange={setIsSummaryOpen}>
        <DialogContent className="p-8 bg-white rounded-xl shadow-lg max-w-2xl mx-auto">
          <ScrollArea className="max-h-[70vh] w-full">
            ...
            <DialogHeader className="border-b-2 pb-4 mb-6">
              <DialogTitle className="text-2xl font-bold text-gray-800">
                Résumé des informations Redevence
              </DialogTitle>
            </DialogHeader>
              ...
              <div className="flex items-center gap">
                ...
                <div className="flex-1 text-center">
                  ...
                  <div className="text-sm uppercase my-1">
                    MINISTÈRE DE LA COMMUNICATION, CHARGÉ DES POSTES ET DES
                    TÉLÉCOMMUNICATIONS
                  </div>
                        ...
                        <div className="flex justify-between items-center p-4 bg-gray-50 rounded-md shadow-md">
                          ...
                          <div className="text-sm text-gray-800">
                            ...
                            <div className="text-lg font-semibold">
                              {donnees?.timbre && <span>(inclut timbre)</span>}
                            </div>
                          </div>
                        </div>
                      ) : (

...
const FormSousCOptonal = () => {
  ...
  useEffect(() => {
    ...
    // Filtrer seulement les sous-couvertures valides
    const validSousCouvertures = Array.isArray(sousCouvertures)
      ? sousCouvertures.filter(
          (sc) => sc && sc.societe?.trim() !== "" // tu peux ajouter d'autres champs si nécessaire
        )
      : [];

    if (haveSousCouvert) {
      total += montantBaseSc * validSousCouvertures.length;
    }
    ...
    form.setValue("montantSC", montantBaseSc * validSousCouvertures.length);
    form.setValue("montantLd", haveLd ? montantBaseLd : 0);
    form.setValue("montantCll", haveCll ? montantBaseCll : 0);

    // console.log("Sous-couvertures valides:", validSousCouvertures);
  }, [watchedFields]);
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input
                              placeholder="Adresse collection X"
                              {...field}
                            />
                          </FormControl>
                          <FormMessage />
